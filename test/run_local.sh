...
OUTPUT_BASE="$SCRIPT_DIR/outputs/local_run_$TIMESTAMP"
mkdir -p "$OUTPUT_BASE"
...
log_output "boltzGen Pipeline Test - Local UV Environment"
log_output "=============================================="
...
if [ -f "$REPO_ROOT/.venv/bin/activate" ]; then
    ...
    log_output "✓ Virtual environment activated"
else
    log_output "✗ Virtual environment not found at $REPO_ROOT/.venv"
    log_output "  Run: bash setup_uv_env.sh"
    exit 1
fi
...
NANOBODY_YAML="example/nanobody_scaffolds/7eow.yaml"
NANOBODY_OUTPUT="$PIPELINE_OUTPUT/nanobody_7eow"

log_config "Nanobody Test:"
log_config "  YAML: $NANOBODY_YAML"
log_config "  Protocol: nanobody-anything"
log_config "  Output: $NANOBODY_OUTPUT"
...
log_output ""
log_output "Note: The 7eow.yaml uses a simplified format (path-based) rather than entities format."
log_output "This format is valid for redesigning existing structures."
log_output ""

if [ ! -f "$NANOBODY_YAML" ]; then
    log_output "✗ Nanobody YAML not found: $NANOBODY_YAML"
    exit 1
fi

# The 7eow.yaml format needs to be converted to entities format
# Create a converted version in the same directory as original for path resolution
NANOBODY_YAML_CONVERTED="example/nanobody_scaffolds/7eow_converted.yaml"
python3 << 'PYEOF'
import yaml
from pathlib import Path

yaml_path = Path("example/nanobody_scaffolds/7eow.yaml")
    ...
    'entities': [
        {
            'file': {
                ...
                'structure_groups': data.get('structure_groups', []),
                'design_insertions': data.get('design_insertions', []),
                'reset_res_index': data.get('reset_res_index', [])
            }
        }
    ]
}

# Write converted version in the same directory as original for path resolution
converted_path = Path("example/nanobody_scaffolds/7eow_converted.yaml")
with open(converted_path, 'w') as f:
    yaml.dump(entities_format, f, default_flow_style=False, sort_keys=False, allow_unicode=True)
PYEOF

if [ ! -f "$NANOBODY_YAML_CONVERTED" ]; then
    log_output "✗ Failed to convert nanobody YAML"
    exit 1
fi

log_output "Using converted YAML: $NANOBODY_YAML_CONVERTED"
...
boltzgen run "$NANOBODY_YAML_CONVERTED" \
    --output "$NANOBODY_OUTPUT" \
    --protocol nanobody-anything \
    --num_designs 2 \
    --budget 2 \
    --cache "$REPO_ROOT/cache" 2>&1 | tee -a "$EXEC_LOG" || {
    log_output "✗ Nanobody test failed"
    # Clean up converted file
    rm -f "$NANOBODY_YAML_CONVERTED"
    exit 1
}
...
VANILLA_YAML="example/vanilla_protein/1g13prot.yaml"
VANILLA_OUTPUT="$PIPELINE_OUTPUT/vanilla_1g13"

log_config "Vanilla Protein Test:"
log_config "  YAML: $VANILLA_YAML"
log_config "  Protocol: protein-anything"
log_config "  Output: $VANILLA_OUTPUT"
log_config "  Designs: 2"
log_config "  Budget: 2"
log_config ""

if [ ! -f "$VANILLA_YAML" ]; then
    log_output "✗ Vanilla YAML not found: $VANILLA_YAML"
    exit 1
fi
...
# Count CIF files
NANOBODY_CIFS=$(find "$NANOBODY_OUTPUT/final_ranked_designs" -name "*.cif" 2>/dev/null | wc -l)
VANILLA_CIFS=$(find "$VANILLA_OUTPUT/final_ranked_designs" -name "*.cif" 2>/dev/null | wc -l)
